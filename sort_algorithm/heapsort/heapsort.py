...
class Heap():
    ...
    def heapsort(self):
        ...
        for i in range(len(self.A), 1, -1):  # 从len(B)到2
            self.A[0], self.A[i - 1] = self.A[i - 1], self.A[0]  # 交换第1个元素和第i个元素
            self.heap_size -= 1
            self.max_heaplify(1)
        return self.A
    ...
    def min_heaplify(self, i):
        ...
        if R <= self.heap_size and self.A[R - 1] < self.A[smallest - 1]:
            smallest = R
        if smallest != i:
            self.A[i - 1], self.A[smallest - 1] = self.A[smallest - 1], self.A[i - 1]
    ...
    def heap_increase_key(self, i, key):
        ...
        while i > 1 and self.A[self.PARENT(i) - 1] < self.A[i - 1]:
            self.A[i - 1], self.A[self.PARENT(i) - 1] = self.A[self.PARENT(i) - 1], self.A[i - 1]
            i = self.PARENT(i)
        return self.A
